var Api = new Class({
	...
	initialize: function(url, cb_wrap)
	{
		this.api_url = url;
	...
	set_auth: function(username, auth_key)
	{
	...
	_call: function(api_url, method, resource, data, params)
	{
		...
		if(params.auth) {
			...
			auth_key = params.auth.auth;
		}
		if(username, auth_key && send_auth) {
			request.headers['Authorization'] = 'Basic ' + btoa(username+':'+auth_key);
		}
		return Sexhr(request)
			...
			.catch(function(e) {
				...
				else if(!params.response_type && xhr)
				{
					try
					{
						var json = decode_msg();
				...
				var why = res && res.error ? res.error.message : (disconnected ? 'disconnected' : (timed_out ? 'timed out' : 'unknown reason'));
