	...
	var success = function(res)
	{
		if(['create', 'update', 'delete'].contains(method))
		{
			...
			if(!options.skip_remote_sync)
			{
				...
				var sync_method = options.custom_method || method;

				// let sync know we have an outgoing change!
				turtl.sync.queue_outgoing_change(table, sync_method, sync_data);
			}
		}
...
var RemoteSync = function(action, model, options) {
	...
	if(['create', 'update'].indexOf(action) >= 0) {
		data = model instanceof Protected ?
			model.safe_json() :
			model.toJSON();
	}

	if(method != 'get' && options.skip_remote_sync) return options.success();
