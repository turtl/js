var PagesController	=	Composer.Controller.extend({
	...
	pre_route: function(url)
	{
		...
		if(this.base_url)
		{
			var match	=	research.router.find_matching_route(url, config.rewrite);
			if(match)
			{
		...
		var last	=	(research.last_url || window.location.pathname).replace(/\-\-.*/, '');
		var cur		=	url.replace(/\-\-.*/, '');
		if(last != cur)
		{
	...
	post_load: function()
	{
		...
		// scroll_to_top is set in the musio::route_callback function in research.js
		//console.log('scroll');
		if(research.scroll_to_top)
		{
			window.scrollTo(0, 0);
			research.scroll_to_top	=	false;
		}
	...
	render: function(data, layout)
	{
		...
		this.el.inject(research.main_container);
		return this;
	},
